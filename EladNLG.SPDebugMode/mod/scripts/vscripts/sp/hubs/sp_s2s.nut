...
void function MaltaHangar_Main( entity player )
{
	...
	if ( enemies.len() > numAlive )
	{
		thread DebugMode_FlagSetOn_DeadOrLeechedObjective( "hangarLaunchTicks", enemies, enemies.len() - numAlive )
		thread DebugMode_FlagSetOn_DeadOrLeechedObjective( "hangarbackup3", enemies, enemies.len() - numAlive )
	}
    ...
    thread DebugMode_DeadOrLeechedObjective( "allEnemies", enemies, enemies.len() )
	numAlive = 4
	if ( enemies.len() > numAlive )
...
// NOTE: This manipulates the array and removes dead entities. Don't care.
// anti-mem leak fr fr
void function DebugMode_FlagSetOn_DeadOrLeechedObjective( string flag, array<entity> entArray, int num )
{
    bool createObjective = flag != "hangarLaunchTicks"
    if (createObjective)
        DebugMode_CreateObjective( flag, "hangarEnemyWave", num, 0 )
    int target = entArray.len() - num
    while ( true )
    {
        WaitFrame()

        ArrayRemoveDeadOrLeeched( entArray )
        if (createObjective)
            DebugMode_SetProgress( flag, entArray.len() - target )
        
        if (entArray.len() < target)
            break
    }
    FlagSet( flag )
}

void function DebugMode_DeadOrLeechedObjective( string group, array<entity> entArray, int num )
{
    DebugMode_CreateObjective( group, "hangarEnemyWave", entArray.len(), 0 )
    int target = entArray.len() - num
    while ( true )
    {
        WaitFrame()
        ...
        ArrayRemoveDeadOrLeeched( entArray )
        DebugMode_SetProgress( group, entArray.len() - target )
