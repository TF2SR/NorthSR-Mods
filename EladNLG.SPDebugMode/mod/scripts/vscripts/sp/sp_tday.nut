...
void function IntroBattleThink( entity player )
{
	...
	string objectiveId = "tdayWaves"
	
    // isComplete - are all enemies spawned
    // progress - wave number
    // maxProgress - wave count
	DebugMode_CreateObjective( objectiveId, "tdayWaves", enemyTitans.len(), 1 )
	...
	while( true )
	{
		...
		DebugMode_SetProgress( objectiveId, enemyTitans.len() - 3 )
	...
	// update wave and tell debugmode we're spawning enemies
	DebugMode_SetProgress( objectiveId, -1 )
	DebugMode_SetMaxProgress( objectiveId, 2 )
	...
	DebugMode_SetMaxProgress( objectiveId, 3 )
	//DebugMode_SetProgress( objectiveId, -1 )
	...
	waitthread WaitUntilDoorsShouldOpen( player, enemyTitans, objectiveId )
...
void function WaitUntilDoorsShouldOpen( entity player, array<entity> enemyTitans, string objectiveId )
{
	...
	while ( true )
	{
		...
		ArrayRemoveDead( enemyTitans )
		DebugMode_SetProgress( objectiveId, enemyTitans.len() - 2 )
	...
	// we set wave to 4 to let debugmode know we're about to early out
	DebugMode_SetMaxProgress( objectiveId, 4 )
