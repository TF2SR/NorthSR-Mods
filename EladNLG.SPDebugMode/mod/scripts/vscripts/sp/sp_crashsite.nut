...
void function WaitForEnemyCount( int count = 0, float timeout = 9999 )
{
	...
	string id = "waitForEnemyCount" + calls++
	DebugMode_CreateObjective_EnemyWave( id, file.waveEnemyCount - count, endTime )

	while( Time() < endTime )
	{
		DebugMode_SetProgress( id, float( file.waveEnemyCount - count ) )
